...
const DashboardSidebar = ({ collapsed, onToggle, onSignOut }) => {
    return (
        <aside className={`flex flex-col bg-slate-950 text-white transition-all duration-300 ${collapsed ? 'w-full lg:w-[96px]' : 'w-full lg:w-[268px]'}`}>
            ...
            <div className="border-t border-white/10 px-4 py-5">
                <div className={`rounded-2xl bg-white/5 ${collapsed ? 'flex justify-center px-0 py-4' : 'flex items-center gap-3 px-4 py-4'}`}>
                    <div className="flex h-12 w-12 items-center justify-center rounded-full border-2 border-amber-300 bg-white text-slate-950 font-bold">
                        U
                    </div>
                    {!collapsed && (
                        <div className="min-w-0 flex-1">
                            <p className="truncate text-sm font-semibold text-white">User account</p>
                    ...
                    {!collapsed && (
                        <button
                            type="button"
                            onClick={onSignOut}
                            className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-slate-300 transition-colors hover:text-white"
                            title="Sign out"
                        >
                            <LogOut size={18} />
                        </button>
